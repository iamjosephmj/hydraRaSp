...
abstract class GenerateHydraSecretsTask : DefaultTask() {
    ...
    fun generate() {
        ...
        sb.append("    }\n\n")
        sb.append("    /**\n")
        sb.append("     * Decrypted bytes of the named encrypted asset (see hydra { encryptAssets }).\n")
        sb.append("     * SWEEP-GATED exactly like {@link #secret}: BLOCKS until the first clean\n")
        sb.append("     * sweep, so call it OFF the main thread. On a compromised device the\n")
        sb.append("     * process is killed first and the plaintext never materialises. Throws if\n")
        sb.append("     * the name is unknown or the ciphertext fails its integrity check.\n")
        sb.append("     */\n")
        sb.append("    public static byte[] asset(Context ctx, String name) {\n")
        sb.append("        return io.ssemaj.deviceintelligence.internal.AssetDec.g(ctx, name);\n")
        sb.append("    }\n")
        sb.append("}\n")
